...
var (
	defaultAlertManagerName              = "obs-alertmanager"
	defaultAlertmanagerObjectMeta        = metav1.ObjectMeta{Name: defaultAlertManagerName}
	alertmanagerTestName                 = "test-alert-manager-name"
	alertmanagerServiceAccountAnnotation = map[string]string{"serviceaccounts.openshift.io/oauth-redirectreference.primary": "{\"kind\":\"OAuthRedirectReference\",\"apiVersion\":\"v1\",\"reference\":{\"kind\":\"Route\",\"name\":\"obs-alertmanager\"}}"}
...
func TestAlertManagerResources_GetDefaultNameAlertmanager(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "return `obs-alertmanager` if NOT self contained and spec AlertManagerDefaultName is empty",
			args: args{
